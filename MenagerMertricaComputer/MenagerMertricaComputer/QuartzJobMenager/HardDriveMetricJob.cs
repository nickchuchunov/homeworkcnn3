...
namespace MenagerMertricaComputer
{
    public class HardDriveMetricJob : IJob
    {
        ...
        HardDriveMetricJob(AcessAgent1 acessAgent1)
        {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            string SQLCommand = "SELECT MAX(Time) FROM harddrivemetrica";
            ...
            IList<HddMenegerMetrica> df = _acessAgent1.GetAllHddMetrics(new GetAllHddMetricsApiRequest { StartTimr = toParameter, EndTime = fromParameter });


            //Записать df в базу данных

            foreach (HddMenegerMetrica df1 in df)
            {
                string ConnectionString = "Data Source = metricsmeneger.db; Version = 3; Pooling = true; Max Pool Size = 100;";
                ...
                cmd.CommandText = "INSERT INTO harddrivemetrica(Id, Value, Time) VALUES(@Id, @Value, @Time)";
